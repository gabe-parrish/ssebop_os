#### Author: Matt Schauer
#### Affiliation: Innovate!, Inc. - TSSC Contractor to the U.S. Geological Survey
#### Origin: U.S. Geological Survey Earth Resources and Observation Science Center (EROS) Center
####        Sioux Falls, South Dakota, United States, 57198
### Email: mschauer@contractor.usgs.gov

### Name: Landsat_SSEBop_ETa.py
### Python Version: 2.7
### Requirements: ArcGIS/arcpy Installation with 'Spatial Analyst' Extension
...
directory = raw_input("Enter filepath to Landsat files:")
#directory = r'Y:\Users\Matt\Iraq\Analysis\p169r37'
...
for i in tarfiles:
    ...
    print 'Landsat',str(landsatNumber),'Image'
    ...
    print 'Preprocessing of Landsat scene complete.'
    print 'Starting ETf/ETa SSEBop model.'
    ...
    print "finding the Tmax, dT, and ETo inputs"
    ...
    def cfactor(ndviRas,lstRas):
        ...
        if mu > 0.0:
            ...
            print 'cfactor algorithm failed. using the default cfactor of 0.985'
            cfactor = float(0.985)
        logfile = open(output + os.sep + 'Cfactors' + '.txt','a')
    ...
    def ssebopETf(lstRas,cfactor):
        ...
        # SSEBop Model
        print 'calculating SSEBop ET Fraction'
        Tcold = tmax * cfactor
        Thot = Tcold + dtcon2
    ...
    def ssebopETa(etf,k):
        print "calculating actual ET with k-factor of",str(k)
        ETa = etf * (eto * float(k))
    ...
    print 'Created ETf file.'
    etf = arcpy.Raster(output + os.sep + 'ETf' + os.sep + 'etf' + scenedate + '.tif')    
    maskedETa = ssebopETa(etf,k)
    outETa = outputfile(maskedETa,'ETa')
    print 'Created ETa file.'
    print "SSEBop ET Fraction and Actual ET created"
    
    timeEnd = str(datetime.now() - SceneStartTime)    
    print 'Total Time to complete scene:',str(timeEnd)

# Delete the Scratch folder and intermediate data
print 'deleting the scratch folder'
shutil.rmtree(directory + os.sep + 'scratch')
