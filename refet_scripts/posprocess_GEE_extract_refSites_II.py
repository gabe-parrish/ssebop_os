...
for modis_f, sn in zip(modis_files_list, snames):
    ...
    ax.set_ylabel('NDVI')
    plt.show()
    plt.savefig(os.path.join(plot_output, f'NDVI_drought_timeseries_{sn}_USDMlvl{USDM_drought_threshhold}.PNG'))
    ...
    if len(USDM_drought_brackets)>0:
        # ================= USDM Drought =====================
        fig, ax = plt.subplots()
        ax.scatter(USDM_drought_df[USDM_drought_df['drought_veg_ref']]['daily_station_eto'],
