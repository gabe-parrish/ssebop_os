...
"""Here we compare a bunch of extracted (see BulkComparisonNV_GM_extract) DRI agrimet sites in Nevada to
 Extracted Gridmet via plotting with matplotlib This was originally lumped in under BulkComparisonNV.py but was 
 separated out due its distinct purpose. The script also outputs some files useful for analysis like accumulated 
...
if not os.path.exists(yearly_output):
    os.mkdir(yearly_output)
monthly_output = r'Z:\Users\Gabe\refET\DRI_Grid_monthly_compare'
if not os.path.exists(monthly_output):
    os.mkdir(monthly_output)
daily_output = r'Z:\Users\Gabe\refET\DRI_Grid_daily_compare'
...
for mp, gmp, cn in zip(metpaths, gridmet_path, common_names):
    ...
    # smoothing = False because we want to aggregate so keep original data
    ...
    m_df = dri_preprocess(mp, interpolate=False)
    # m_df = dri_preprocess(mp, interpolate=True, terp_lim=None)

    # save the original df before you do the reformat
    m_orig_df = m_df
    ...
    # add the DRI ETos back on there for use later
    m_df = pd.concat([m_orig_df['ET_total_ASCE'], m_orig_df['ET_Penman_total'], m_df], axis=1)
    ...
    daily_merge.to_csv(os.path.join(daily_output, '{}.csv'.format(cn)))
