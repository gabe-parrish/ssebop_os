...
for modis_f, sn in zip(modis_files_list, snames):
    ...
    continuous_df = daily_ts_df[daily_ts_df['ETo_Station_Linear'].notnull()]
    ...
    try:
        yearly_ts = continuous_df.groupby(pd.Grouper(freq="A")).agg(MaxAir=pd.NamedAgg(column='MaxAir', aggfunc=np.nanmean),
                                                             ...
                                                             ETo_Station_Linear=pd.NamedAgg(column='ETo_Station_Linear',
                                                                                           aggfunc=np.nansum),
                                                             ETo_Station_Var=pd.NamedAgg(column='ETo_Station_Linear',
                                                                                        aggfunc=np.nanvar),
    ...
    plt.plot(yearly_ts.index, yearly_ts['ETo_Station_Linear'], color='green', label='Cumulative Station ETo')
    plt.scatter(yearly_ts.index, yearly_ts['ETo_Station_Linear'], color='green', facecolor=None)
